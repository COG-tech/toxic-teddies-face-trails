...
- Main commit before branch:
- Current `NEXT_ACTION.md` reviewed: yes / no
...
- [ ] `npm run check`
- [ ] `npm run build`
- [ ] Browser `/play/` bundle generated when player-facing behavior changed.
- [ ] Android build run when native/bundle behavior changed.
- [ ] iOS build run when native/bundle behavior changed.
...
Human/device/artwork/research/store gates still pending:

- [ ] No physical, accessibility, participant-research, signing, artwork-approval, or store-upload work is claimed complete without evidence.

## Second-brain updates

- [ ] `STATUS.md` updated if current truth changed.
...
- Next action:
- Published-build verification required:
