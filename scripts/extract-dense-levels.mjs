...
function radial(cell, size) {
  const half = (size - 1) / 2;
  return Math.hypot(cell[0] - half, cell[1] - half);
}
...
function orientationOptions(pathCells, size) {
  const half = (size - 1) / 2;
...
function generate(level) {
  ...
  while (available.size >= 2 && added.length < config.maxPieces) {
    ...
    for (const seedKey of candidates.slice(0, Math.min(500, candidates.length))) {
      for (const targetLength of [12, 10, 8, 6, 5, 4, 3, 2]) {
        ...
        for (const option of orientationOptions(pathCells, config.size)) {
  ...
  if (!verifySolution(added, solutionOrder, config.size)) {
    throw new Error(`Level ${level} failed head-ray solver verification`);
  }
  ...
  return {
    ...
    decorations: [],
    visualAnchors: ['torn circular ears', 'button eye', 'infected eye', 'forehead seam', 'muzzle and black nose', 'cheek patch', 'expression mouth', 'radioactive slime'],
    quality: {
      coverage: Number(coverage.toFixed(3)),
      verifiedSolvable: true,
      startingOpenPieces,
      startingBlockedPieces: added.length - startingOpenPieces,
      solver: 'reverse_construction_head_ray_v1',
    },
    animation: {
      pauseMs: 90,
      baseSlideMs: 420,
...
console.log(`Compiled ${manifest.length} deterministic Toxic Toby levels (${manifest.reduce((sum, item) => sum + item.pieceCount, 0)} paths).`);
