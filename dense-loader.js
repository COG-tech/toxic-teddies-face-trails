(async () => {
  try {
    const [baseResponse, denseResponse, fallbackResponse, interactionResponse] = await Promise.all([
      fetch('./compiled-app.js?v=9', { cache: 'no-store' }),
      fetch('./hard-mode-v3.js?v=9', { cache: 'no-store' }),
      fetch('./dense-fallback.js?v=9', { cache: 'no-store' }),
      fetch('./interaction-fix.js?v=9', { cache: 'no-store' }),
    ]);
