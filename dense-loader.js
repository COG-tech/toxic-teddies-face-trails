(async () => {
  try {
    const [baseResponse, denseResponse, fallbackResponse, interactionResponse] = await Promise.all([
      fetch('./compiled-app.js?v=28', { cache: 'no-store' }),
      fetch('./hard-mode-v3.js?v=28', { cache: 'no-store' }),
      fetch('./dense-fallback.js?v=28', { cache: 'no-store' }),
      fetch('./interaction-fix.js?v=28', { cache: 'no-store' }),
    ]);
